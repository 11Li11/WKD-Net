...
from model.mobilemamba.mobilemamba import MobileMambaBlock
...
class MASPP(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels, mid_channels, 1, bias=False), nn.GroupNorm(4, mid_channels),
                                   nn.GELU())
        self.ac1 = self._make_layer(mid_channels, mid_channels, dilation=1)
        self.ac2 = self._make_layer(mid_channels * 2, mid_channels, dilation=3)
        self.ac3 = self._make_layer(mid_channels * 3, mid_channels, dilation=5)
        self.ac4 = self._make_layer(mid_channels * 4, mid_channels, dilation=7)
